...
export const sampleDataService = {
  ...
  async initializeAllSampleData(targetDate: string = new Date().toISOString().split('T')[0]): Promise<InitializationResult> {
    try {
      ...
      const responseData = customersResponse.data.data;
      const existingCustomers: Customer[] = responseData.customers || responseData.data || [];
      console.log(`✅ Found ${existingCustomers.length} existing customers`);

      let technicians = existingTechnicians;
      ...
      const jobs = await this.createSampleJobs(customers, technicians, targetDate);
      ...
      return {
        ...
        jobs,
        summary: {
          techniciansCreated: technicians.length,
